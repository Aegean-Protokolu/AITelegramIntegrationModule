...
class MemoryArbiter:
    ...
    def consider(
        ...
        print(f"🔍 [Arbiter] Processing: type={mem_type}, subject={subject}, confidence={confidence}, text={text[:50]}")

        if mem_type not in TYPE_PRECEDENCE:
            print(f"❌ [Arbiter] Type '{mem_type}' not in precedence table")
            return None

        # 1️⃣ Confidence gate
        min_conf = CONFIDENCE_MIN[mem_type]
        if confidence < min_conf:
            print(f"❌ [Arbiter] Confidence gate failed: {confidence} < {min_conf} (required for {mem_type})")
            return None

        print(f"✅ [Arbiter] Passed confidence gate: {confidence} >= {min_conf}")
        ...
        if previous_versions:
            ...
            if last_text == text.strip():
                print(f"❌ [Arbiter] Exact duplicate detected (ID: {previous_versions[-1]['id']})")
                return None

        print(f"✅ [Arbiter] No duplicates. Previous versions: {len(previous_versions)}, parent_id: {parent_id}")

        # 3️⃣ Conflict detection (exact, conservative)
        conflicts = self.memory_store.find_conflicts_exact(text)
        print(f"🔍 [Arbiter] Found {len(conflicts)} conflicts")
        ...
        for c in conflicts:
            if TYPE_PRECEDENCE[c["type"]] > TYPE_PRECEDENCE[mem_type]:
                ...
                print(f"⚠️ [Arbiter] Dampening confidence due to conflict with {c['type']}: {confidence} -> {adjusted_confidence}")

        # 5️⃣ Append-only write (versioned)
        print(f"✅ [Arbiter] Saving memory with adjusted_confidence={adjusted_confidence}")
